...
Class Phieuluong extends MY_Controller
{
    function __construct()
    {
        parent::__construct();
        $this->load->model('phieuluong_model');
...
//            pre($rows);
            ...
            $employee = $this->phieuluong_model->demo($employee, $from, $to);
...
//            $objPHPExcel->getActiveSheet()->getRowDimension(1)->setRowHeight(5);
            ...
            $sheet2->setCellValueByColumnAndRow(1, 4, "Từ " . date('d/m/Y', $from) . ' đến ' . date('d/m/Y', $to));
            $sheet->getStyle('B3:D3')->applyFromArray($center);
...
//            $sheet->mergeCells("G".($row_count+1).":I".($row_count+4));
            ...
            $sheet->getStyle('I' . ($i + 1) . ':J' . ($i + 1))->applyFromArray($center);

            $objPHPExcel->setActiveSheetIndex(0)->mergeCells('I' . ($i + 2) . ':J' . ($i + 2));
            $sheet->getStyle('I' . ($i + 2) . ':J' . ($i + 2))->applyFromArray($center);
...
//            end công phát sinh
            $j++;
            $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(0, $j, 'Tổng Lương thực');
            $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(3, $j, number_format($total + $total_cps));
            $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(4, $j, 'vnđ');

//            $i++;
            $objWriter = PHPExcel_IOFactory::createWriter($objPHPExcel, 'Excel5');
            header("Content-Type: application/vnd.ms-excel");
            header('Content-Disposition: attachment; filename="phieuluong.xls"');
...
//        /export data
        ...
        $this->data['temp'] = 'admin/phieuluong/phieuluong';
...
//    excel
    public function excel()
    {
        ...
        $objPHPExcel->getActiveSheet()->SetCellValue('A1', 'Địa điểm');
        $objPHPExcel->getActiveSheet()->SetCellValue('B1', 'Số NC');
        $objPHPExcel->getActiveSheet()->SetCellValue('C1', 'Lương');
        $objPHPExcel->getActiveSheet()->SetCellValue('D1', 'Tổng lương');
