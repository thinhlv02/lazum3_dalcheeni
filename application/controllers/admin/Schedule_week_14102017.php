...
//            'role' => 2
//        );
//        $info = $this->employee_model->get_info(13);
//        pre($info->name);
...
//            $objPHPExcel->getActiveSheet()->getRowDimension(1)->setRowHeight(5);
                ...
                $sheet->setCellValueByColumnAndRow(2, 2, "LỊCH DẠY PHÂN BỔ GIÁO VIÊN " . date('d/m', $week[0]->mon) . ' - ' . date('d/m', $week[0]->sun));
...
//                    pre($itemMonId);
                    ...
                    if ($itemMonId && $itemMonId[0]) {
                        foreach ($itemMonId as $row) {
                            ...
                            $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(1, $i, $itemMon->start . ' / ' . $info->name);
                        }
                    }
                    ...
                    if ($itemTueId && $itemTueId[0]) {
                        foreach ($itemTueId as $row) {
                            $itemMon = $this->teach_model->get_info($row);
                            $info = $this->employee_model->get_info($itemMon->employee_id);
                            $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(2, $i, $itemMon->start . ' / ' . $info->name);
                        }
                    }
                    ...
                    if ($itemWedId && $itemWedId[0]) {
                        foreach ($itemWedId as $row) {
                            $itemMon = $this->teach_model->get_info($row);
                            $info = $this->employee_model->get_info($itemMon->employee_id);
                            $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(3, $i, $itemMon->start . ' / ' . $info->name);
                        }
                    }
                    ...
                    if ($itemThuId && $itemThuId[0]) {
                        foreach ($itemThuId as $row) {
                            $itemMon = $this->teach_model->get_info($row);
                            $info = $this->employee_model->get_info($itemMon->employee_id);
                            $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(4, $i, $itemMon->start . ' / ' . $info->name);
                        }
                    }
                    ...
                    if ($itemFriId && $itemFriId[0]) {
                        foreach ($itemFriId as $row) {
                            $itemMon = $this->teach_model->get_info($row);
                            $info = $this->employee_model->get_info($itemMon->employee_id);
                            $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(5, $i, $itemMon->start . ' / ' . $info->name);
                        }
                    }
                    if ($itemSatId && $itemSatId[0]) {
                        foreach ($itemSatId as $row) {
                            $itemMon = $this->teach_model->get_info($row);
                            $info = $this->employee_model->get_info($itemMon->employee_id);
                            $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(6, $i, $itemMon->start . ' / ' . $info->name);
                        }
                    }
                    ...
                    if ($itemSunId && $itemSunId[0]) {
                        foreach ($itemSunId as $row) {
                            $itemMon = $this->teach_model->get_info($row);
                            $info = $this->employee_model->get_info($itemMon->employee_id);
                            $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(7, $i, $itemMon->start . ' / ' . $info->name);
                        }
                    }
                    /*borders*/
                    $BStyle = array(
...
//                                'color' => array('rgb' => '6665ff')
                    ...
                    $objPHPExcel->getActiveSheet()->getStyle('A' . ($i) . ':H' . ($i))->applyFromArray($BStyle);
                    /*borders*/
                    $i++;
                }
...
//
//            } else {
//                $this->session->set_flashdata('message', 'Bị trùng lịch');
...
//            if ($this->teach_model->demo($emp, $branch, $start, $end)) {
    ...
    function edit()
    {
        ...
        $schedule_week_id = $this->uri->segment(4);
        $schedule_week_id = intval($schedule_week_id);
        //pre($schedule_week_id);
        ...
        if ($this->input->post('btnUpdateschedule_week')) {
            ...
            if ($this->schedule_week_model->update($schedule_week_id, $dataSubmit)) {
                $this->session->set_flashdata('message', 'Sửa thông tin huấn luyện viên, diễn viên thành công!');
                redirect(base_url('admin/schedule_week'));
            } else {
                $this->session->set_flashdata('message', 'Sửa thông tin huấn luyện viên, diễn viên thất bại!');
                redirect(base_url('admin/schedule_week'));
            }
        }

        $this->data['temp'] = 'admin/schedule_week/edit';
        $this->load->view('admin/layout', $this->data);
    }
