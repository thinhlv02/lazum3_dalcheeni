...
Class Bangchamcong_kd extends MY_Controller
{
    function __construct()
    {
        parent::__construct();
        $this->load->model('bangchamcong_kd_model');
...
//                $objPHPExcel->setActiveSheetIndex(0)->mergeCells('A' . ($i) . ':C' . ($i));
                ...
                foreach ($arr_branch_id as $key => $value1) {
                    $total = '';
                    $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(0, $i, $this->branch_model->get_info($value1)->name);
                    $dem = '';
...
//                pre($row);
            ...
            header('Content-Disposition: attachment; filename="LuongHLV_KD.xls"');
...
//        /export data
        ...
        $this->data['temp'] = 'admin/bangchamcong/bangchamcong_kd';
