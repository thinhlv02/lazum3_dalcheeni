...
<div class="page-title">
    <div class="title_left"><h3>Phiếu lương cá nhân</h3></div>
...
<div class="x_panel">
    ...
    <!--    công phát sinh-->
    <div class="x_content">
        <h4 class="btn btn-success">Công phát sinh/ tiền phạt</h4>
        <!--        <a href="-->
        <?php //echo admin_url('phieuluong/excel') ?><!--" class="btn btn-success btn-lg">Export excel</a>-->
        <table id="datatable-product" class="table table-striped table-bordered bulk_action">
            <thead>
            ...
            <tbody>
            <?php $i = 0; ?>
            <?php if (isset($congphatsinh) && count($congphatsinh) > 0) { ?>
            <?php foreach ($congphatsinh as $key => $value): ?>
                <?php $i++; ?>
                <tr>
                    <td><?php echo $i ?></td>
                    <td><?php echo $value->name ?></td>
                    ...
                    <td><?php if ($value->type == 0) {
                            echo number_format($value->money);
                        } else {
//                            echo '-' . number_format($value->money);
                            echo '<span class="maudo">- ' . number_format($value->money) . '</span>';
                        } ?></td>
                    <td><?php if ($value->type == 0) {
                            echo 'Tiền phát sinh';
                        } else {
            ...
            <tfoot>
            <tr style="background-color: rgba(43, 138, 120, 0.27);">
                <td colspan="3" style="text-align: center">Tổng còn:</td>
                <td><?php echo number_format($total_cps) ?></td>
            </tr>
            </tfoot>
            <?php } ?>
        </table>
    </div>
    <h1>Tổng lương: <span class="maudo"><?php echo number_format($total + $total_cps) ?></span> (vnđ) </h1>
    <!--    công phát sinh-->
</div>
